...
<body>
    ...
    <div>
        @if (Auth::check() && Auth::user()->role == 'admin')
        <td><a href='{{route("sub-classifications.create")}}' class="btn btn-primary">Add a New Sub Classification</a></td>
        @endif
    </div>
    <div class="card-body">
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $sc->name }}</td>
                    <td>{{ $sc->mainClassification->name }}</td>
                    @if (Auth::check() && Auth::user()->role == 'admin')
                    <td><a href='{{route("sub-classifications.edit", $sc->id)}}' class="btn btn-outline-warning">EDIT</a></td>
                    <td>
                        <form action="{{ route('sub-classifications.destroy', $sc->id) }}" method="POST">
                            @csrf
                            @method ('DELETE')
